...
function create($title,$content,$image){
    $pdo =getPDO();
    $query = $pdo->prepare('INSERT INTO posts(title,content,image,created_at) VALUES(:title,:content,:image,NOW())');
    $query->execute(compact('title','content','image'));
    $id = $query->insert_id;
    return $id;
}
...
function updatePost($id,$author,$title,$content,$image){
    $pdo =getPDO();
    $query = $pdo->prepare('UPDATE posts SET  title = :title,content = :content,image = :image WHERE id =:id'); 
    $query->execute(compact('title','content','image','id'));
    $id ->insert_id;
    return $id;
}
